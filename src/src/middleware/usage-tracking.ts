/**
 * Usage Tracking Middleware for Nexus-Robotics
 *
...
async function flushBatch(): Promise<void> {
  ...
  console.log(`[Usage Tracking] Flushing usage batch: ${batch.length} reports`);
...
function detectOperation(req: Request): string {
  ...
  // Robotics API operations
  if (path.includes('/missions')) {
    if (path.includes('/execute')) return 'mission_execute';
    return 'mission_management';
  }
  if (path.includes('/telemetry')) return 'telemetry_ingest';
  if (path.includes('/sensors')) return 'sensor_fusion';
  if (path.includes('/objects')) {
    if (path.includes('/classify')) return 'object_classification';
    if (path.includes('/detect')) return 'object_detection';
    return 'object_operation';
  }
  if (path.includes('/safety')) return 'safety_monitoring';
  if (path.includes('/navigation')) return 'navigation';
  if (path.includes('/swarm')) return 'swarm_coordination';
  if (path.includes('/platforms')) return 'platform_management';
  if (path.includes('/commands')) return 'command_execution';
...
async function sendUsageReport(report: UsageReport, retryCount = 0): Promise<void> {
  ...
  try {
    ...
    console.warn('[Usage Tracking] Error response', {
    ...
    if (!errorMessage.includes('abort')) {
      console.warn('[Usage Tracking] Failed to send usage report', { error: errorMessage });
    }
  ...
  res.on('finish', () => {
    ...
    if (USAGE_TRACKING_CONFIG.enableBatching) {
      ...
      sendUsageReport(report).catch((err) => {
        console.error('[Usage Tracking] Unexpected error', {
...
export async function flushPendingReports(): Promise<void> {
  console.log('[Usage Tracking] Flushing pending usage reports', { queueSize: usageQueue.length });
  await flushBatch();
}
